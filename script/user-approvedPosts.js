const resultDisplayDiv = document.querySelector(".approved-posts-div");
...
const createAllAds = (data) => {
  ...
  if (data.length > 0) {
    ...
    message.innerText = "You Currently Have No Approved Post";
...
fetch(url, {
  ...
  .then((data) => {
    if (data.status === "error") {
      ...
      createAllAds(data.Approved);
      approvedPostsCount.innerText = data.Approved.length;
