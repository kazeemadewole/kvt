const resultDisplayDiv = document.querySelector(".rejected-posts-div");
const rejectedPostsCount = document.querySelector(".rejected-posts-count");
...
const createAllAds = (data) => {
  ...
  if (data.length > 0) {
    ...
    message.innerText = "You Currently Have No Rejected Post";
...
fetch(url, {
  ...
  .then((data) => {
    if (data.status === "error") {
      ...
      createAllAds(data.Rejected);
      rejectedPostsCount.innerText = data.Rejected.length;
